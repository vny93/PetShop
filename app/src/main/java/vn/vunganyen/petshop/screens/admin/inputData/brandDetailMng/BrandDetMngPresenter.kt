...
class BrandDetMngPresenter {
    ...
    fun validCheck(req : PostBrandReq){
        if (req.tenhang.isEmpty() || req.email.isEmpty() || req.sdt.isEmpty() || req.mota.isEmpty()){
    ...
    fun updateInforBrand(token: String,req : PostBrandReq){
        ApiBrandService.Api.api.updateInforBrand(token,req).enqueue(object : Callback<PostBrandRes>{
            override fun onResponse(call: Call<PostBrandRes>, response: Response<PostBrandRes>) {
                if(response.isSuccessful){
                    //getProfile(token)
    ...
    fun checkUri(uri: Uri, token: String, req: PostBrandReq) {
        if (uri != null) {
            var strRealPath: String = RealPathUtil().getRealPath(BrandDetailMngActivity.context, uri)
            ...
            ApiBrandService.Api.api.updateLogoBrand(token, id,multipart).enqueue(object : Callback<PostBrandRes> {
                override fun onResponse(call: Call<PostBrandRes>,response: Response<PostBrandRes>)
                {
                    if (response.isSuccessful) {
